...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof maxBy, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 3', function test( t ) {
	t.strictEqual( maxBy.length, 3, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the maximum value of an array via a callback function', function test( t ) {
	var x;
	var v;
...
tape( 'the function calculates the maximum value of an array via a callback function (accessors)', function test( t ) {
	var x;
	var v;

	x = [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ];
	v = maxBy( toAccessorArray( x ), accessor );
	t.strictEqual( v, 10.0, 'returns expected value' );
...
tape( 'if provided an empty array, the function returns `NaN` (accessors)', function test( t ) {
	var v = maxBy( toAccessorArray( [] ), accessor );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
	t.end();
});
...
tape( 'if provided an array containing a single element, the function returns the result of applying a provided callback function to that element (accessors)', function test( t ) {
	var v = maxBy( toAccessorArray( [ 1.0 ] ), accessor );
	t.strictEqual( v, 2.0, 'returns expected value' );
	t.end();
});
